...
public class Day1 : ProblemSet
{
    ...
    private static string ReplaceNumbers(string line)
    {
        ...
        for (var i = 0; i < line.Length; i++)
        {
            ...
            {
                if (substring.Length < mapping.Item1.Length)
                    continue;

                if (!substring.StartsWith(mapping.Item1))
                    continue;

                var end = substring.Length > mapping.Item2.Length
                    ? substring.Substring(mapping.Item2.Length)
                    : "";
                line = start + mapping.Item2 + end;
            }
        }
